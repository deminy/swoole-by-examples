...
response (Bad Request):
    docker exec -t  $(docker ps -qf "name=client") bash -c "curl -i http://server:9503"
...
If you only want to check if the WebSocket server is running or not, you can do it with following curl command:
    docker exec -ti $(docker ps -qf "name=client") bash -c \
        'curl -iN -H "Connection: Upgrade" -H "Upgrade: websocket" -H "Sec-WebSocket-Key: dummy" http://server:9503'
